function PathAnime() {
    ...
    var exports= function (selection) {
        for (var i = 0, latlngs = [], len = _route.length; i < len; i++) {
            latlngs.push(new L.LatLng(_route[i][0], _route[i][1]));
        }
        _path = L.polyline(latlngs, _style);
       ...
       _map.fitBounds(L.latLngBounds(latlngs));
        ...
        _path.bindTooltip({permanent: true, opacity: 0.4, offset: new L.Point(100, 200)})
            .setTooltipContent("<div>"+_info.route+"</div>").openTooltip();

        _path.on('snakeend', function(ev){
            _path.bindPopup(_info.note.news).openPopup();
        });
    }
